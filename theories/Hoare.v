...
Notation "a [<>] b" := (fun st => mkAssertStack a st <> mkAssertN b st)
  (at level 70, no associativity) : assertion_scope.
...
Definition assertion_push_stack (val : N) (P : Assertion) : Assertion :=
  fun (st : state) =>
    P {| stack := val :: st.(stack); mem := st.(mem); dict := st.(dict) |}.
Notation "P '[.S' :: v ]" := (assertion_push_stack v P)
  (at level 10) : hoare_spec_scope.
...
Notation "P '[WORDS' a := p ]" := (assertion_sub_stack a p P)
  (at level 10) : hoare_spec_scope.
